<template>
    <div class="textInputWrapper" :style="{ '--accent-color': accentColor }">
        <input :placeholder="inputPlaceholder" type="text" class="textInput" v-model="inputValue"
            @input="$emit('input', inputValue)" />
        <!-- Clipboard icon, initially hidden, shows on hover or focus -->
        <span class="input-icon" @click="pasteClipboard"><i class="pi pi-clipboard" style="font-size: 1rem"></i></span>
    </div>
</template>
