<template>
    <div class="grid lg:grid-rows-2 lg:grid-cols-3 h-[100%] gap-1 ml-0 mr-2 mt-2">
        <!-- TOP ROW (Top 3) -->
        <div v-for="(value, key, index) in symbolsTop" :key="index" class="flex flex-col items-center ">
            <!-- header section -->
            <div class="flex justify-between items-center w-full bg-blue-600 rounded h-[40px]">
                <div :id="'ticker-name-top-' + index" class="font-bold ml-3 font-serif rounded text-white ">
                    {{ symbolsTopLabels[key] }}
                </div>

                <!-- Timeframe -->
                <div class="mx-auto max-w-xs">
                    <select :id="'timeframe-select-top-' + index"
                        ...
                        text-black
                    "
                        v-model="timeframesTop[index]"
                        @change="changeTimeframe(index, $event.target.value)">
                ...
                <div class="dropdown dropdown-hover">
                    ...
                    <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-40 p-2 shadow">
                        <li class="text-black font-thin hover:bg-sky-500 border-b-2 border-solid border-gray-300"
                            v-for="symbol in availableSymbols" :key="symbol.value"
                            @click="updateChart(index, 'top', symbol.value, symbol.label)">
                            <a>{{ symbol.label }}</a>
                        </li>
                    </ul>
                </div>
            ...
            <!-- Chart Section -->
            <div :id="'chart-top-' + index" class="w-full h-[450px] rounded "></div>
        </div>

        <!-- BOTTOM ROW (Bottom 3) -->
        <div v-for="(value, key, index) in symbolsBottom" :key="index" class="flex flex-col items-center ">
            <!-- header section -->
            <div class="flex justify-between items-center w-full bg-blue-600 p-2 rounded">
                <div :id="'ticker-name-bottom-' + index" class="font-bold ml-3 font-serif rounded text-white ">
                    {{ symbolsBottomLabels[key] }}
                </div>

                <!-- Dropdown for selecting symbols -->
                <div class="dropdown dropdown-hover">
                    <div tabindex="0" role="button" class="btn m-1 btn-sm">Select</div>
                    <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-40 p-2 shadow">
                        <li class="bg-black" v-for="symbol in availableSymbols" :key="symbol.value"
                            @click="updateChart(index, 'bottom', symbol.value, symbol.label)">
                            <a>{{ symbol.label }}</a>
                        </li>
                    </ul>
                </div>
            ...
            <!-- Chart Section -->
            <div :id="'chart-bottom-' + index" class="w-full h-full rounded h-[450px]"></div>
        </div>
    </div>
</template>
<script>
export default {
    data() {
        return {
            ...
            timeframesTop: ['1', '1', '1'], // Default to 1 minute for each top chart
        };
    },
    methods: {
